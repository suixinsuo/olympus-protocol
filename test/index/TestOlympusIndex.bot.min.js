...
contract("Olympus Index Bot Reduced", accounts => {
  ...
  it("buy tokens, withdraw and rebalance", async () => {
    ...
    console.log("status after selling: ", (await index.getProductStatus()).toNumber());
    ...
    await Promise.all(
      investorsGroupA.map(async account => {
        ...
        console.log({ beforeBalance: amount.toNumber() });
    ...
    await Promise.all(
      investorsGroupA.map(async account => {
        const amount = (await index.balanceOf(account)).toNumber();
        console.log({ afterBalance: amount });
      })
    );
    ...
    console.log("status: ", (await index.getProductStatus()).toNumber());
    assert.isAbove(price, web3.toWei(1, "ether"), "Price is above 1");
    assert.isBelow(price, web3.toWei(1.1, "ether"), "Price is less than 1.1");
