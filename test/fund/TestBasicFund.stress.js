...
contract("BasicFund", accounts => {
  ...
  it("Create a fund, the price should be 1 ether", async () => {
    fund = await Fund.new(
      fundData.name,
      fundData.symbol,
  ...
  it("Five investor invest with different amount", async () => {
    ...
    await fund.invest({
      value: web3.toWei(50.1, "ether"),
      from: investorA
    });

    await fund.invest({
      value: web3.toWei(21.3, "ether"),
      from: investorB
    });

    await fund.invest({
      value: web3.toWei(35.6, "ether"),
      from: investorC
    });

    await fund.invest({
      value: web3.toWei(22, "ether"),
      from: investorD
    });

    await fund.invest({
      value: web3.toWei(21.50, "ether"),
      from: investorE
    });
  });

  it("Total assets is 0.", async () => {
    const assetsValue = await fund.getAssetsValue();
    // console.log('assetsValue:', assetsValue.toNumber());
    // console.log('toWei:', web3.toWei(1, "ether")); 
    assert.equal(assetsValue, web3.toWei(0, "ether"));
  });


  it("TotalSupply shall be equal with invested.", async () => {
    const total = 50.1 + 21.3 + 35.6 + 22 + 21.50;
    assert.equal((await fund.totalSupply()).toNumber(), web3.toWei(total, "ether"), "Supply is updated");
    // Price is the same, as no Token value has changed
    assert.equal((await fund.getPrice()).toNumber(), web3.toWei(1, "ether"));

    assert.equal((await fund.balanceOf(investorA)).toNumber(), toTokenWei(50.1));
    assert.equal((await fund.balanceOf(investorB)).toNumber(), toTokenWei(21.3));
  });

  it("Balance shall be equal with invested.", async () => {
    const total = 50.1 + 21.3 + 35.6 + 22 + 21.50;
    const initialBalance = (await web3.eth.getBalance(fund.address)).toNumber();
    assert.equal(initialBalance, web3.toWei(total, "ether"), "This test must start with total invested eth");
  });
  ...
  it("Shall be able to buy tokens", async () => {
    ...
    const remains = (await web3.eth.getBalance(fund.address)).toNumber();

    assert.equal(remains, web3.toWei(149.5, "ether"),
      "ETH balance reduced");

    const assetsValue = await fund.getAssetsValue();
    // console.log('assetsValue:', assetsValue.toNumber());
    // console.log('toWei:', web3.toWei(1, "ether")); 
    assert.equal(assetsValue, web3.toWei(1, "ether"));
  });

  it("Price shall changed after token's price changed", async () => {
  ...
  //   );
  //   // We sell all
  //   tx = await fund.sellTokens("", fundTokensAndBalance[0], fundTokensAndBalance[1], sellRates.map(rate =>
  //     rate[0]));
  ...
  //   assert.equal((await web3.eth.getBalance(fund.address)).toNumber(), web3.toWei(1.8, "ether"),
  //     "ETH balance reduced");
  // });
  ...
  //   assert.equal((await fund.balanceOf(investorA)).toNumber(), toTokenWei(0),
  //     "Investor redeemed all the funds");
  ...
  //   assert.equal((await web3.eth.getBalance(fund.address)).toNumber(), web3.toWei(2, "ether"),
  //     "ETH balance returned");
  //   await calc.assertReverts(async () => await fund.changeStatus(DerivativeStatus.Active),
  //     "Shall not be  close");
