...
contract("ComponetList", accounts => {
  ...
  it("Set and Get ", async () => {
    ...
    await list.setComponent("reimbursable", mockComponent.address, { from: accounts[0] });
    const component = await list.getComponent("reimbursable", await mockComponent.version());
  ...
  it("Override component", async () => {
    ...
    await list.setComponent("reimbursable", olderMockComponent.address, { from: accounts[0] });
    let component = await list.getComponent("reimbursable", await olderMockComponent.version());
    ...
    await list.setComponent("reimbursable", newMockComponent.address, { from: accounts[0] });
    component = await list.getComponent("reimbursable", await newMockComponent.version());
    assert.equal(
      component,
  ...
  it("update version", async () => {
    ...
    // set olderMockComponent's address into componentList contract.
    await list.setComponent("reimbursable", mockComponentV1.address, { from: accounts[0] });
    await list.setComponent("reimbursable", mockComponentV2.address, { from: accounts[0] });
    //mockComponentV1
    let component = await list.getComponent("reimbursable", await mockComponentV1.version());
    assert.equal(component, mockComponentV1.address, "mockComponentV1's address is correct");
    //mockComponentV2
    component = await list.getComponent("reimbursable", await mockComponentV2.version());
  ...
  it("permission check", async () => {
    ...
    try {
      await list.setComponent("reimbursable", mockComponentV2.address, { from: accounts[1] });
  ...
  it("get latest version", async () => {
    ...
    // set olderMockComponent's address into componentList contract.
    await list.setComponent("reimbursable", mockComponentV1.address, { from: accounts[0] });
    let component = await list.getLatestComponent("reimbursable");
    let versions = (await list.getComponentVersions("reimbursable")).map(bytes32ToString);
    ...
    await list.setComponent("reimbursable", mockComponentV2.address, { from: accounts[0] });
    versions = (await list.getComponentVersions("reimbursable")).map(bytes32ToString);
    assert.deepEqual(versions, ["V1", "V2"]);
    component = await list.getLatestComponent("reimbursable");
    assert.equal(
      await MockComponent.at(component).version(),
