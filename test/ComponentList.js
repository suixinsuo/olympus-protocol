...
contract('ComponetList', (accounts) => {
  ...
  it("Override component", async () => log.catch(async () => {
    ...
    assert.equal(component, newMockComponent.address, 'Override the component successfully: it is the same with newMockComponent');
    assert.notEqual(component, olderMockComponent.address, 'Override the component successfully: olderMockComponent is different with the address from component list');
  }));

  it("update version", async () => log.catch(async () => {
