...
contract TokenBroken is TokenBrokenInterface {
    ...
    function calculateBalanceByInvestor(ERC20Extended _token) external view returns(uint[]) {
        address[] memory investors = MappeableDerivative(msg.sender).getActiveInvestors();
        uint[] memory balances = new uint[](investors.length);
        ...
        for (uint i = 0; i <= investors.length; i++) {
          ...
          // balances[i] = ERC20Extended(msg.sender)
          //   .balanceOf(investors[i]);
