...
contract ExchangeProvider is Ownable, Provider {
    ...
    uint constant EXCHANGE_ID_OFFSET = 1000;

    ExchangeAdapter[] exchanges;
    mapping (uint=>uint) balance; // orderId => eth
    ...
    function _makeExchangeId(uint index) private returns(uint) {
    ...
    function _pickExchange(address token, uint rate) private returns (uint exchangeIndex, bool found) {
    ...
    function placeOrder(uint orderId, address[] tokenAddresses, uint[] amount, uint[] rates, address deposit) external payable returns (bool success) {
