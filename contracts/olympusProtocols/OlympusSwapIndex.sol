...
contract OlympusSwapIndex is IndexInterface, Derivative {
   ...
   public onlyOwner payable {
        ...
        bytes32[9] memory names = [
            MARKET, EXCHANGE, REBALANCESWAP, WHITELIST, FEE, REIMBURSABLE, WITHDRAW, LOCKER, STEP
        ];
  ...
  // solhint-disable-next-line
    ...
    function rebalance() public onlyOwnerOrWhitelisted(WhitelistKeys.Maintenance) whenNotPaused returns (bool success) {
        ...
        (_tokensToSell,_tokensToBuy,_amounts) = rebalanceSwapProvider.rebalanceGetTokensToTrade(rebalanceDeltaPercentage);
        ...
        for (i = currentStep; i < _tokensToSell.length && goNextStep(REBALANCE); i++) {
            ...
            require(
                exchangeProvider.tokenExchange(
                    ERC20Extended(_tokensToSell[i]),
                    ERC20Extended(_tokensToBuy[i]),
                    _amounts[i],
                    0,
                    address(this),
                    0x0),
