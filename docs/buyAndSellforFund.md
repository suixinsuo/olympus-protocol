...
Sending MOT directly the fund will wont affect the price.

### Interface

Thats the basic functions of the interfaces, that allow to buy and sell.
...
Once a token has been buy or sold, the fund will automatically update his internal state and the result of this function.

WARNING! When a token is completely sold, still will be return in the addresses array with the amount of 0.
...
Firs we set the tokens and amounts, we can select a rate with `0` value, or find the current one
...
using the previus function `getTokens()`.
You can specify the exchangeId of the provider, but in most scenarios leaving at `0x0` allowing our component to chose the best provider for you.
The list of tokens and the list of amounts must be of the same size, bieng the amounts the value on tokens unit (take in account
...
In a real situation, buy or sell tokens is convert your ETH in assets values or opposite, that shoudnt change the price of the fund. In a realistic situation,
